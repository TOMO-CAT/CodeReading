...
用法: python3 scripts/gen_blade_build.py [-h] [-r] folder_path
参数:
...
def gen_blade_deps(file_list, binary=False, is_main_binary=False):
    ...
    for header_file in header_files:
        ...
        if not os.path.exists(header_folder):
            ...
            candidate_header_file_list = [item for item in candidate_header_file_list if not item.startswith("thirdparty/blade-build")]
