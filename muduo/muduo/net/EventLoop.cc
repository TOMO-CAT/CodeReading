...
// 防止同一个线程创建多个 EventLoop, 当某个 EventLoop 被创建时, t_loopInThisThread 就会置为这个 EventLoop 对象的指针
// 当 t_loopInThisThread 非空时就不允许当前线程再创建 EventLoop 了
...
void EventLoop::loop() {
  ...
  while (!quit_) {
    ...
    // TODO(cat): sort channel by priority
    eventHandling_ = true;
