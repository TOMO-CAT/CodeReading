...
#define storeBSwap(bits) memcpy(target, &source, sizeof(source));
...
std::streamsize Data::deserialize(int64_t& pos, std::basic_streambuf<char>* streamBuffer, std::streamsize bytes) {
  ...
  if (pos >= 0) {
    ...
    if (dataLen > 0) {
      if (!data) data.reset(new char[dataLen]);
...
void Binary::toJSON(std::ostream& stream) const {
  ...
  for (uint32_t i = 0; i < len; ++i) stream << (uint16_t)data[i] << " ";
  stream << ">";
}
...
void Extended::toJSON(std::ostream& stream) const {
  ...
  for (uint32_t i = 1; i < len; ++i) stream << (uint16_t)data[i] << " ";
  stream << ">";
}
...
int64_t String::getEndPos() const {
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXSTR && type < Type::NIL) return type - Type::FIXSTR;
...
int64_t String::getHeaderLength() const {
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXSTR && type < Type::NIL) return 1;
...
int64_t Number::getEndPos() const {
  ...
  if (type < 0x80 || type >= 0xE0) return 1;
...
uint32_t ArrayHeader::getLength() const {
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXARRAY && type < Type::FIXSTR) return type - Type::FIXARRAY;
...
int64_t ArrayHeader::getHeaderLength() const {
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXARRAY && type < Type::FIXSTR) return 1;
...
int64_t MapHeader::getHeaderLength() const {
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXMAP && type < Type::FIXARRAY) return 1;
...
uint32_t MapHeader::getLength() const {
  uint8_t type = static_cast<const uint8_t>(header[0]);
  if (type >= Type::FIXMAP && type < Type::FIXARRAY) return type - Type::FIXMAP;
...
std::unique_ptr<Element> Array::copy() const {
  ...
  for (const auto& i : elements) _elements.push_back(std::unique_ptr<Element>(i->copy()));
...
uint32_t Array::getSizeInBytes() const {
  uint32_t size = getHeaderLength(), len = elements.size();
  for (uint32_t i = 1; i < len; ++i) size += elements[i]->getSizeInBytes();
  return size;
}
...
Map::Map(std::vector<std::unique_ptr<Element>>&& _elements)
  ...
  if (elements.size() % 2 == 1) elements.erase(elements.end() - 1);
}

Map::Map(std::map<std::string, std::unique_ptr<Element>>&& _elements) : MapHeader(_elements.size()) {
...
std::pair<String*, Element*> Map::getEntry(uint32_t index) const {
  ...
  if (index + 1 < elements.size()) {
    ...
    if (key) return std::pair<String*, Element*>(key, elements[index + 1].get());
  }
...
std::streamsize Serializer::serialize(PullCallback pullElement, std::streamsize bytesLeft) {
  ...
  while (bytesLeft > 0 || deserializeAll) {
    ...
    if (stack.size() == 0) {
      ...
      if (!rootElement) break;  // Got no element: quit
    ...
    if (bytesWritten == 0) break;  // Stream overflow: quit

    if (stackPointer->second < stackPointer->first->getEndPos()) continue;  // Not done yet
    ...
    if (stackIndex == 0) rootElement.reset();
  }
...
std::streamsize Deserializer::deserialize(PushCallback pushElement, bool hierarchy, std::streamsize bytesLeft) {
  ...
  while (bytesLeft > 0 || deserializeAll) {
    ...
    if (stack.size() == 0 && pushElement(std::move(rootElement))) break;  // One element done: quit
  }

  return bytesDone;
}

std::streamsize Deserializer::deserialize(std::unique_ptr<Element>& element, bool hierarchy,
                                          std::streamsize bytesLeft) {
  return deserialize(
      [&element](std::unique_ptr<Element> parsedElement) {
