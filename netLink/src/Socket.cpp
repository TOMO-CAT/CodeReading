...
void init() {
  WSADATA wsaData;
  if (WSAStartup(MAKEWORD(2, 0), &wsaData) != 0) throw Exception(Exception::ERROR_INIT);
}
...
Socket::AddrinfoContainer Socket::getSocketInfoFor(const char* host, unsigned int port, bool wildcardAddress) {
  ...
  if (wildcardAddress) conf.ai_flags |= AI_PASSIVE;
  switch (type) {
    case TCP_CLIENT:
    case TCP_SERVER:
  ...
  if (result != 0) throw Exception(Exception::ERROR_RESOLVING_ADDRESS);
...
int Socket::sync() {
  ...
  if (pptr() == pbase())  // Allready in sync
    return 0;
  try {
    std::streamsize rest = pptr() - pbase(), sentBytes = send(pbase(), rest);
    rest -= sentBytes;
...
Socket::int_type Socket::underflow() {
  if (type == UDP_PEER || advanceInputBuffer() <= 0) return EOF;
  return *eback();
}
...
void Socket::initSocket(bool blockingConnect) {
  ...
  while (nextAddr) {
    ...
    if (blockingConnect) setBlockingMode(false);
    break;
  }
  if (handle == -1) {
    disconnect();
...
Socket::Socket() : ipVersion(ANY), type(NONE), status(NOT_CONNECTED), handle(-1), portLocal(0), portRemote(0) {
}
...
std::streamsize Socket::receive(char_type* buffer, std::streamsize size) {
  if (type == TCP_SERVER) throw Exception(Exception::BAD_TYPE);
  if (status != Socket::Status::READY && status != Socket::Status::BUSY) return 0;
  ...
  switch (type) {
    ...
    case TCP_SERVERS_CLIENT: {
      ...
      if (result <= 0) throw Exception(Exception::ERROR_READ);
      return result;
    }
    default:
    case NONE:
...
std::streamsize Socket::send(const char_type* buffer, std::streamsize size) {
  if (type == TCP_SERVER) throw Exception(Exception::BAD_TYPE);
  if (status != Socket::Status::READY || size == 0) return 0;
  switch (type) {
    case UDP_PEER: {
      ...
      while (sentBytes < (size_t)size) {
        int result =
            ::sendto(handle, (const char*)buffer + sentBytes, size - sentBytes, 0, info->ai_addr, info->ai_addrlen);
        if (result <= 0) {
          status = BUSY;
...
std::streamsize Socket::redirect(const std::vector<std::shared_ptr<Socket>>& destinations) {
  ...
  while (in_avail()) {
    ...
    for (const auto& destination : destinations)
      if (destination->sputn(gptr(), length) < length) throw Exception(Exception::ERROR_SEND);
    gbump(length);
    size += length;
    advanceInputBuffer();
  }
...
void Socket::setBroadcast(bool active) {
  if (type != UDP_PEER || ipVersion != IPv4) throw Exception(Exception::BAD_PROTOCOL);
#ifdef WINVER
  char flag = 1;
#else
  int flag = 1;
...
void Socket::setMulticastGroup(const std::string& address, bool join) {
  if (type != UDP_PEER) throw Exception(Exception::BAD_PROTOCOL);
  if (ipVersion == IPv4) {
    struct in_addr sin;
    if (inet_pton(AF_INET, address.c_str(), &sin) == -1) throw Exception(Exception::ERROR_RESOLVING_ADDRESS);
    ...
    struct in6_addr sin;
    if (inet_pton(AF_INET6, address.c_str(), &sin) == -1) throw Exception(Exception::ERROR_RESOLVING_ADDRESS);
