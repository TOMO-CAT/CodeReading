...
#define foreach_e(c, i)                                                                                            \
  for (auto end##i = (c).end(), next##i = (c).begin(), i = (next##i == end##i) ? end##i : next##i++; i != next##i; \
...
#define forEachSocket()                                                \
  ...
  if (socket->getStatus() == Socket::Status::NOT_CONNECTED) removeSocket()
...
void SocketManager::listen(double waitUpToSeconds) {
...
#endif
    if (select(maxHandle + 1, NULL, &writefds, NULL, timeoutPtr) == -1) throw Exception(Exception::ERROR_SELECT);

  foreach_e(selection, iterator) {
    forEachSocket()

        if (socket->type == Socket::Type::TCP_SERVER) continue;
    ...
    if (onStatusChange && socket->status != prev) onStatusChange(this, *iterator, prev);
    checkSocketStillValid(sockets, iterator, socket)

        // Try to send data of MsgPack queue in socket
        if (socket->status != Socket::Status::READY) continue;
  ...
  if (select(maxHandle + 1, &readfds, NULL, NULL, timeoutPtr) == -1) throw Exception(Exception::ERROR_SELECT);

  foreach_e(selection, iterator) {
    forEachSocket()

        if (!FD_ISSET(socket->handle, &readfds)) continue;
    ...
    if (socket->type == Socket::Type::TCP_SERVER) {
      ...
      if (socket->type == Socket::Type::UDP_PEER) socket->advanceInputBuffer();
      // Received new data
