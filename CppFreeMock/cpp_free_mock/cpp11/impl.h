...
#include "cpp_free_mock/cpp11/basic_type.h"
...
//      a) Because the entry point is a static function, so the mocked method should be static or invoke from a static
//      variable. b) Because we can mock member function, so the mocked method must be a member function, so we can only
//      use a static variable to call it.
...
struct Mocker<I(R(P...))> : MockerBase<R(P...)> {
  ...
  Mocker(FunctionType function, const std::string& functionName)
      : MockerBase<FunctionType>(functionName), originFunction(function) {
    SimpleSingleton<decltype(this)>::getInstance() = this;
    RuntimePatcher::GraftFunction(originFunction, MockerEntryPoint<IntegrateType>::EntryPoint,
...
#define MOCKER_WITH_THIS_POINTER_CHECK(constness)                                                  \
  ...
  struct Mocker<I(R (C::*)(const void*, P...) constness)> : MockerBase<R(const void*, P...)> {     \
    ...
    Mocker(FunctionType function, const std::string& functionName)                                 \
      ...
      SimpleSingleton<decltype(this)>::getInstance() = this;                                       \
      RuntimePatcher::GraftFunction(originFunction, &MockerEntryPoint<EntryPointType>::EntryPoint, \
...
struct MockerCreator {
 private:
  ...
  template <typename I, typename R, typename... P>
  static const std::shared_ptr<MockerBase<R(P...)>> CreateMocker(R function(P...), const std::string& functionName) {
...
#define CREATE_MOCKER_WITH_THIR_POINTER_CHECK(constness)                                                         \
  template <typename I, typename C, typename R, typename... P>                                                   \
  static const std::shared_ptr<MockerBase<R(const void*, P...)>> CreateMocker(R (C::*function)(P...) constness,  \
                                                                              const std::string& functionName) { \
    typedef I IntegrateType(R (C::*)(const void*, P...) constness);                                              \
...
#undef CREATE_MOCKER_WITH_THIR_POINTER_CHECK
 ...
 public:
  template <typename I, typename R, typename... P>
  static const std::shared_ptr<MockerBase<R(P...)>> GetMocker(R function(P...), const std::string& functionName) {
    return DoGetMocker<I, MockerBase<R(P...)>>(function, functionName);
  }

#define GET_MOCKER_WITH_THIR_POINTER_CHECK(constness)                                                   \
  template <typename I, typename C, typename R, typename... P>                                          \
  static std::shared_ptr<MockerBase<R(const void*, P...)>> GetMocker(R (C::*function)(P...) constness,  \
                                                                     const std::string& functionName) { \
