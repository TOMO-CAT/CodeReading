 ...
 *   - By default, logs will be written into a local file. Users can set a
 * callback to write logs to different destinations.
 *
 * @param cb
 ...
 *
 * @param cb
 *   a callback takes 3 params:  void f(const char* topic, const void* p, size_t
 * n);
 ...
 */
__coapi void
set_write_cb(const std::function<void(const char *, const void *, size_t)> &cb,
             int flags = 0);
...
public:
  TLogSaver(const char *fname, unsigned fnlen, unsigned line,
            const char *topic);
  ~TLogSaver();
...
#define DLOG                                                                   \
  if (FLG_min_log_level <= log::xx::debug)                                     \
  _CO_LOG_STREAM(log::xx::debug)
#define LOG                                                                    \
  if (FLG_min_log_level <= log::xx::info)                                      \
  _CO_LOG_STREAM(log::xx::info)
#define WLOG                                                                   \
  if (FLG_min_log_level <= log::xx::warning)                                   \
  _CO_LOG_STREAM(log::xx::warning)
#define ELOG                                                                   \
  if (FLG_min_log_level <= log::xx::error)                                     \
  _CO_LOG_STREAM(log::xx::error)
...
#define CHECK(cond)                                                            \
  if (!(cond))                                                                 \
  _CO_FLOG_STREAM << "check failed: " #cond "! "

#define CHECK_NOTNULL(p)                                                       \
  if ((p) == 0)                                                                \
  _CO_FLOG_STREAM << "check failed: " #p " mustn't be NULL! "

#define _CO_CHECK_OP(a, b, op)                                                 \
  for (auto _x_ = std::make_pair(a, b); !(_x_.first op _x_.second);)           \
  _CO_FLOG_STREAM << "check failed: " #a " " #op " " #b ", " << _x_.first      \
                  << " vs " << _x_.second << "! "
...
#define _CO_LOG_EVERY_N(n, what)                                               \
  ...
  if (atomic_fetch_inc(&_CO_LOG_COUNTER, mo_relaxed) % (n) == 0)               \
  what

#define _CO_LOG_FIRST_N(n, what)                                               \
  static int _CO_LOG_COUNTER = 0;                                              \
  if (_CO_LOG_COUNTER < (n) &&                                                 \
      atomic_fetch_inc(&_CO_LOG_COUNTER, mo_relaxed) < (n))                    \
  what
