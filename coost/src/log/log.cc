  ...
  if (level != xx::fatal) {
    ...
    if (!new_file) {
      if (!_old_paths.empty()) {
        ...
        if (fs::fsize(_path) >= FLG_max_log_file_size || (FLG_log_daily && get_day_from_path(path) != _day)) {
...
class Logger {
 ...
 private:
  ...
  LogTime& _time;
  LogFile& _file;
  int _stop;  // -2: init, -1: starting, 0: running, 1: stopping, 2: stopped, 3:
              // final
};

Logger::Logger(LogTime* t, LogFile* f) : _llog(), _tlog(), _log_event(true, false), _time(*t), _file(*f), _stop(-2) {
...
int StackTrace::backtrace(const char* file, int line, const char* func, int& count) {
  ...
  _s.clear();
  _s << '#' << (count++) << "  in " << (func ? func : "???") << " at " << (file ? file : "???") << ':' << line << '\n';
...
void ExceptHandler::handle_signal(int sig) {
  ...
  if (_stack_trace) _stack_trace->dump_stack(&f, m.check_failed ? 7 : (sig == SIGABRT ? 4 : 3));
  if (f) {
    f.write('\n');
    f.close();
  }
...
LevelLogSaver::LevelLogSaver(const char* fname, unsigned fnlen, unsigned line, int level) : _s(log_stream()) {
  _n = _s.size();
